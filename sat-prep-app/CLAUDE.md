...
First is the SAT_Questions directory. This is currently used for SAT questions, but will eventually be ported to work for other types of tests. It begins with a division of a test into all of its subtopics with sample questions. Then a PREAM optimization is used to get the prompt to generate questions that are similar to the test. Semantic Similarity is used instead of numbers and verbalized sampling is used to truly generate different types of questions. 

The second is the sat-prep-app. This is the bread and butter money generator. Questions are generated from here and put into a convex database. 

It has two parts to it: 
- The admin page 
- The actual dashboard for users 

The admin page is necessary to allow me to see the questions, their correct answers, and their explanations. I will need to be able to make a lot of edits from herre and see how certain students are doing on certain questions. This way if a question is wrong too often, I can make a social media video about it.


The dashboard for users will have two main features: 
1) the ability to take a full length test
2) the ability to do endless mode, where questions will be queued for them based on how they are doing 
...
Critical Workflows: 
 - Generated questions must be reviewed before appearing in the frontend. Users cannot see unverified questions. There is a question Review pipeline 
...
- TODO: Edits certain grammar questions so that they can be shown to the frontend! Currently the grammar passages don't have any type of indication that there is supposed to be underlined etc .
