...
exports.QUESTION_TYPE_PROMPTS = exports.READING_SAMPLING_PARAMS = exports.DISTRACTOR_COMBOS_BY_SUBTOPIC = exports.READING_DISTRACTOR_STRATEGIES = exports.PASSAGE_TYPE_CHARACTERISTICS = exports.PASSAGE_TYPES = void 0;
exports.sampleGaussian = sampleGaussian;
exports.sampleFrom = sampleFrom;
exports.sampleDistractorCombo = sampleDistractorCombo;
exports.sampleReadingParams = sampleReadingParams;
exports.buildPassageGenerationPrompt = buildPassageGenerationPrompt;
exports.buildDistractorInstructions = buildDistractorInstructions;
exports.buildQuestionGenerationPrompt = buildQuestionGenerationPrompt;
// ─────────────────────────────────────────────────────────
// PASSAGE TYPES
// ─────────────────────────────────────────────────────────
exports.PASSAGE_TYPES = [
    'literary_narrative',
    'social_science',
    'natural_science',
    'humanities',
];
exports.PASSAGE_TYPE_CHARACTERISTICS = {
    literary_narrative: {
...
// ─────────────────────────────────────────────────────────
exports.READING_DISTRACTOR_STRATEGIES = {
    too_broad: "Answer is technically true but too general. It could apply to many passages and doesn't specifically address the question.",
    too_narrow: "Answer focuses on a minor detail that's in the passage but misses the main point. True statement, wrong scope.",
    opposite_meaning: "Answer reverses the author's actual position or the passage's meaning. Contradicts what the text says.",
    unsupported_inference: 'Answer makes a reasonable-sounding claim that goes beyond what the text actually supports. Plausible but not evidenced.',
    wrong_scope: 'Answer references content from the wrong paragraph or applies an idea from one part to a question about another part.',
    misread_tone: "Answer misinterprets the author's attitude, confusing approval with criticism, certainty with hesitation, etc.",
    partial_answer: 'Answer addresses only part of what the question asks. Incomplete even if partially correct.',
    plausible_but_wrong: "Answer uses language/phrases from the passage but draws an incorrect conclusion. Sounds right, isn't right.",
    extreme_position: "Answer uses absolute language ('always', 'never', 'completely') when the passage is more nuanced.",
    temporal_confusion: 'Answer confuses sequence of events, or attributes to one time period what belongs to another.',
};
exports.DISTRACTOR_COMBOS_BY_SUBTOPIC = {
    central_ideas: [
...
// ─────────────────────────────────────────────────────────
exports.READING_SAMPLING_PARAMS = {
    ...
    passageLengths: ['short', 'medium', 'long'],
    passageLengthWords: {
...
function sampleDistractorCombo(subtopic) {
    const combos = exports.DISTRACTOR_COMBOS_BY_SUBTOPIC[subtopic] || [
        ['too_broad', 'too_narrow', 'opposite_meaning'],
    ];
    return sampleFrom(combos);
}
function sampleReadingParams(subtopic, overrides) {
    return {
        passageType: overrides?.passageType ?? sampleFrom(exports.PASSAGE_TYPES),
        passageLength: overrides?.passageLength ?? sampleFrom(exports.READING_SAMPLING_PARAMS.passageLengths),
        passageComplexity: overrides?.passageComplexity ??
            sampleGaussian(exports.READING_SAMPLING_PARAMS.passageComplexity.mean, exports.READING_SAMPLING_PARAMS.passageComplexity.stdDev),
        inferenceDepth: overrides?.inferenceDepth ??
            sampleGaussian(exports.READING_SAMPLING_PARAMS.inferenceDepth.mean, exports.READING_SAMPLING_PARAMS.inferenceDepth.stdDev),
        vocabularyLevel: overrides?.vocabularyLevel ??
            sampleGaussian(exports.READING_SAMPLING_PARAMS.vocabularyLevel.mean, exports.READING_SAMPLING_PARAMS.vocabularyLevel.stdDev),
        evidenceEvaluation: overrides?.evidenceEvaluation ??
            sampleGaussian(exports.READING_SAMPLING_PARAMS.evidenceEvaluation.mean, exports.READING_SAMPLING_PARAMS.evidenceEvaluation.stdDev),
        synthesisRequired: overrides?.synthesisRequired ??
            sampleGaussian(exports.READING_SAMPLING_PARAMS.synthesisRequired.mean, exports.READING_SAMPLING_PARAMS.synthesisRequired.stdDev),
...
// ─────────────────────────────────────────────────────────
function buildPassageGenerationPrompt(params) {
    const typeChar = exports.PASSAGE_TYPE_CHARACTERISTICS[params.passageType];
    const lengthRange = exports.READING_SAMPLING_PARAMS.passageLengthWords[params.passageLength];
...
// ─────────────────────────────────────────────────────────
function buildDistractorInstructions(strategies) {
    return strategies
        .map((strategy, i) => `- Choice ${['B', 'C', 'D'][i]} (${strategy}): ${exports.READING_DISTRACTOR_STRATEGIES[strategy]}`)
        .join('\n');
}
exports.QUESTION_TYPE_PROMPTS = {
...
function buildQuestionGenerationPrompt(subtopic, passageAnalysis, distractorStrategies) {
    const basePrompt = exports.QUESTION_TYPE_PROMPTS[subtopic] || exports.QUESTION_TYPE_PROMPTS['central_ideas'];
...
//# sourceMappingURL=reading-templates.js.map