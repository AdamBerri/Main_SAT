...
export class QuestionRepairer {
  ...
  async repair(
    ...
    topic: TopicPath,
  ): Promise<{ question: GeneratedQuestion; result: RepairResult }> {
    ...
    const validation = fullValidation(repaired, topic);
  ...
  async repairBatch(
    ...
    stats: { totalAttempted: number; totalRepaired: number; issueBreakdown: Record<RepairCategory, number> };
  }> {
    ...
    return {
      ...
      stats: {
        ...
        issueBreakdown: issueBreakdown as Record<RepairCategory, number>,
      },
    };
  }
  ...
  private detectLocalIssues(
    question: GeneratedQuestion,
  ): Array<{ category: RepairCategory; detail: string }> {
    const issues: Array<{ category: RepairCategory; detail: string }> = [];
    ...
    for (const choice of wrongChoices) {
      const rationale = question.distractorRationale[choice.label as keyof typeof question.distractorRationale];
      if (rationale) {
  ...
  private async llmRepair(
    question: GeneratedQuestion,
    topic: TopicPath,
    knownIssues: Array<{ category: RepairCategory; detail: string }>,
  ...
  private diffIssues(
    ...
    localIssues: Array<{ category: RepairCategory; detail: string }>,
  ): Array<{ category: RepairCategory; detail: string }> {
    const issues: Array<{ category: RepairCategory; detail: string }> = [];
      ...
      if (char === '"') {
        ...
        while (i < jsonStr.length) {
          ...
          if (c === '\\') {
            result += c;
            i++;
            if (i < jsonStr.length) { result += jsonStr[i]; i++; }
          } else if (c === '"') {
            result += c; i++; break;
          } else if (code < 32) {
            switch (code) {
