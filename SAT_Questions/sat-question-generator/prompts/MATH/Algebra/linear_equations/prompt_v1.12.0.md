...
5. DISTRACTOR RATIONALE FORMAT (ALL 4 KEYS REQUIRED):
...
6. DIFFICULTY CALIBRATION:
   - Match difficulty to actual cognitive demands, not superficial complexity:
     ...
     * Difficulty 4: Multi-step problem with extra information to filter, system of equations with non-obvious setup, or problems requiring unit conversion before solving
     * Difficulty 5: Non-obvious equation setup, multiple layers of abstraction (e.g., percentage changes on top of a system, or interpreting what a derived quantity means). All intermediate and final values must be exact — design the numbers so no rounding is needed at any step.
