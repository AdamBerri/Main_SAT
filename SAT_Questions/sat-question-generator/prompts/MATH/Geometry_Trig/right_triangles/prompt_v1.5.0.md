...
2. PROBLEM STRUCTURE VARIETY (CRITICAL):
   ...
   - AVOID creating two questions that are mirror images of each other (e.g., "given shadow, find height" and "given height, find shadow" with the same angle). Each question must feel distinct.

3. ANGLE VARIETY (IMPORTANT):
   - Within a batch of 5 questions at the same difficulty, use AT LEAST 3 different angle values.
   - Do NOT reuse the same angle (e.g., 37°) in more than 2 out of 5 questions.
   - Vary across angles like 25°, 30°, 35°, 37°, 40°, 45°, 50°, 53°, 55°, 60° and their associated trig values.
   - For angles that produce clean Pythagorean-triple-based trig values: 37° (sin=0.6, cos=0.8), 53° (sin=0.8, cos=0.6), 28° (sin≈0.469, cos≈0.883 — pair with appropriate sides), etc.

4. TRIG VALUE HANDLING (IMPORTANT):
...
6. DISTRACTOR DESIGN (HIGHEST PRIORITY):
   ...
   - NEVER create a distractor that requires the student to first compute the correct answer and THEN make an additional mistake on that result. A student who already computed the correct value would not then perform another operation on it.
   - GOOD: "Used cos instead of sin: cos 37° × 25 = 0.8 × 25 = 20" (one wrong choice of trig function)
   - BAD: "Correctly found opposite = 15, then computed hypotenuse using 15: √(15² + 20²) = 25" (student already found the answer, then did something extra)
   - Each error must be a mistake a real student would make at the START of the problem, not after solving it correctly.
...
8. DISTRACTOR PLAUSIBILITY FLOOR:
...
10. DISTRACTOR RATIONALE FORMAT (ALL 4 KEYS REQUIRED):
...
11. DIFFICULTY CALIBRATION:
   ...
   * Difficulty 3: Two-step problem OR interpret a trig relationship OR complementary angle reasoning. MUST require at least one computational step — a question that only asks the student to recall a definition (e.g., "what does sin θ = 7/25 represent?") is NOT D3, it is D2. Example: "From a point 40 ft from a building, the angle of elevation to the top is 50°. Given tan 50° ≈ 1.19, find the total height if the observer's eyes are 5 ft above the ground."
   * Difficulty 4: Multi-step with 3+ computations, may include extra information that must be identified and ignored. Must combine trig with another concept (area, perimeter, distance). The trig computation itself must be substantive — do NOT use tan(45°) = 1 as the primary trig step since this makes the trig component trivial. Example: "A surveyor measures the angle of elevation to a hilltop as 25° from point A and 40° from point B, which is 200 meters closer. Find the height of the hill."
   * Difficulty 5: Non-obvious setup requiring creative insight, multiple triangles, or working backwards from a derived quantity. MUST go beyond simply combining two standard formulas (e.g., area formula + special triangle ratios alone is NOT sufficient for D5). SPECIFICALLY BANNED for D5: "Given a 45-45-90 triangle with area X, find the hypotenuse" — this is D3 at most. Must require either: (a) setting up and solving a system of equations from two or more geometric relationships, (b) using multiple interconnected triangles with shared sides where information flows between triangles, (c) working backwards from a derived quantity through multiple layers to find an original measurement, or (d) a non-routine geometric insight such as constructing an auxiliary line or recognizing a hidden relationship. All intermediate values exact. Example: "A lighthouse beam rotates. When the angle of depression to a boat changes from 60° to 45°, the boat has moved 50 meters. How tall is the lighthouse?"
