...
11. DIFFICULTY CALIBRATION:
   ...
   * Difficulty 5: Non-obvious setup requiring creative insight, multiple triangles, or working backwards from a derived quantity. MUST go beyond simply combining two standard formulas (e.g., area formula + special triangle ratios alone is NOT sufficient for D5). Must require either: (a) setting up and solving a system of equations, (b) using multiple interconnected triangles with shared sides, (c) working backwards from a derived quantity to find an original measurement, or (d) a non-routine geometric insight. All intermediate values exact. Example: "A lighthouse beam rotates. When the angle of depression to a boat changes from 60° to 45°, the boat has moved 50 meters. How tall is the lighthouse?"
